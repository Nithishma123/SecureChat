...
public class Camellia{
	...
	private static boolean _keyis128;
	private static int [] subkey = new int[24*4];
	private static int [] kw = new int[4*2]; // for whitening
	private static int [] ke = new int[6*2]; // for FL and FL^(-1)
	private static int [] state = new int[4]; // for encryption and decryption
	...
	public static byte lRot8(byte v, int rot) {
		return (byte)((v <<rot) | ((v & 0xff) >>> (8-rot)));
	}

	public static int sbox2(int x) {
		return (int)(lRot8(SBOX1[x], 1) & MASK8);
	}
	public static int sbox3(int x) {
		return (int)(lRot8(SBOX1[x], 7) & MASK8);
	}
	public static int sbox4(int x) {
		return (int)(SBOX1[((int)lRot8((byte)x, 1) & MASK8)] & MASK8);
	}

	private static void camelliaF2(int [] s, int [] skey, int keyoff) {
		int t1, t2, u, v;
	...
	private static void camelliaFLs(int [] s, int [] fkey, int keyoff) {
	...
	private static void setkey(boolean forEncryption, byte [] key) {
		int [] k = new int [8];
		int [] ka = new int [4];
		int [] kb = new int [4];
		int [] t = new int [4];
		...
		if (_keyis128) {
			if (forEncryption) {
				/* KL dependant keys */
				kw[0] = k[0]; kw[1] = k[1]; kw[2] = k[2]; kw[3] = k[3];
				roldq(15, k, 0, subkey, 4);
				roldq(30, k, 0, subkey, 12);
				roldq(15, k, 0, t, 0);
				subkey[18] = t[2]; subkey[19] = t[3];
				roldq(17, k, 0, ke, 4);
				roldq(17, k, 0, subkey, 24);
				roldq(17, k, 0, subkey, 32);
				/* KA dependant keys */
				subkey[0] = ka[0]; subkey[1] = ka[1];
				subkey[2] = ka[2]; subkey[3] = ka[3];
				roldq(15, ka, 0, subkey, 8);
				roldq(15, ka, 0, ke, 0);
				roldq(15, ka, 0, t, 0);
				subkey[16] = t[0]; subkey[17] = t[1];
				...
				/* KL dependant keys */
				kw[4] = k[0]; kw[5] = k[1]; kw[6] = k[2]; kw[7] = k[3];
				...
				/* KA dependant keys */
				subkey[34] = ka[0]; subkey[35] = ka[1];
				subkey[32] = ka[2]; subkey[33] = ka[3];
			...
			if (forEncryption) {
				...
				/* 32bit rotation */
				ke[8]=ka[1];ke[9]=ka[2];ke[10]=ka[3];ke[11]=ka[0];
				roldqo32(49, ka, 0, subkey, 40);

				/* KB dependant keys */
				subkey[0]=kb[0];subkey[1]=kb[1];
				subkey[2]=kb[2];subkey[3]=kb[3];
				...
				/* 32bit rotation */
				ke[2]=ka[1];ke[3]=ka[2];ke[0]=ka[3];ke[1]=ka[0];
				decroldqo32(49, ka, 0, subkey, 4);

				/* KB dependant keys */
				subkey[46]=kb[0];subkey[47]=kb[1];
				subkey[44]=kb[2];subkey[45]=kb[3];
	...
	public static int processBlock128(byte [] in, int inOff,
									  byte [] out, int outOff) {
		for(int i=0; i< 4; i++) {
	...
	public static int processBlock192or256(byte [] in, int inOff,byte [] out, int outOff) {
		for(int i = 0; i < 4; i++) {
			state[i] = bytes2int(in, inOff+(i*4));
			state[i] ^= kw[i];
		}
	...
	public static void init(boolean forEncryption, byte [] key) {
		setkey(forEncryption, key);
		initialized = true;
	}

	public static int processBlock(byte [] in, int inOff,
							byte [] out, int outOff)
    ...
    public static byte[] enc(String st,String key_enc){
      ...
      processBlock(msg,0,out,0);
      //System.out.println("The Message to be Encrypted : ");
      //System.out.println(s);
      //System.out.println("The CipherText : ");
      String cipher = new String(out);

      return out;

}
    
      public static byte[] double_enc(byte[] st,String key_enc){
    ...
    public static String dec(byte[] st, String key_dec){
      ...
      byte[] deci = new byte[100];

      processBlock(out,0,deci,0);
      String d= new String(deci); 
      String decipher = removePadding(d); 
      return decipher;

    }
    
      public static byte[] double_dec(byte[] st, String key_dec){
